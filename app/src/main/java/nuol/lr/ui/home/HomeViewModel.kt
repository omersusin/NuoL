...
class HomeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // YENİ: Çekmecede gösterilecek uygulamalar (Gizli uygulamaları filtreler)
    val filteredApps = combine(_apps, _searchQuery, settingsManager.hiddenAppsFlow) { list, q, hidden -> 
        val visibleApps = list.filter { !hidden.contains(it.packageName) }
        if (q.isBlank()) visibleApps else visibleApps.filter { it.label.contains(q, true) } 
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    // YENİ: Ayarlar menüsünde yönetmek için tüm uygulamaları dışarı aktarır
    val allAppsWithHiddenState = combine(_apps, settingsManager.hiddenAppsFlow) { list, hidden ->
        list.map { Pair(it, hidden.contains(it.packageName)) }
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    val showStatusBar = settingsManager.showStatusBarFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), true) // YENİ
